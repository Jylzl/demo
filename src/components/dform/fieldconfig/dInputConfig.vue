  ...
  <div class="field-attribute">
    ...
    >
      ...
      <el-form-item label="可选组件" v-if="a_type.length">
        <el-select
          v-model="o_config.type"
          placeholder="请选择可选类型"
          @change="$emit('change', o_config)"
        >
          <el-option
            v-for="(item,index) in a_type"
            :key="index"
            :label="item.label"
            :value="item.value"
          ></el-option>
      ...
      <el-row v-if="o_config.type == 'textarea'">
        <el-col :span="12">
          <el-form-item label="高度适应">
            <el-switch v-model="o_config.autosize" @change="$emit('change', o_config)"></el-switch>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="允许缩放">
            <el-switch v-model="o_config.resize" @change="$emit('change', o_config)"></el-switch>
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item label="最小行数" v-if="o_config.type == 'textarea' && !o_config.autosize">
        <el-input-number
      ...
      <el-form-item label="最大行数" v-if="o_config.type == 'textarea' && !o_config.autosize">
        <el-input-number
          v-model="o_config.maxRows"
          controls-position="right"
          :min="o_config.minRows"
          :step="1"
          :max="30"
          class="w100"
          @change="$emit('change', o_config)"
