...
<template>
  ...
  >
    ...
    <el-main class="form-box">
      <el-container>
        ...
        >
          ...
          <div
            class="form-boxwrap"
            :class="{ 'form-bg-one': !myArray.length }"
            ref="formboxwrap"
          >
            <el-scrollbar class="h100">
              <el-form
                ...
                class="widget-form"
              >
                <el-row :gutter="formAttribute.colSpacing">
                  <widgetForm
    ...
    <el-aside width="300px">
      <el-tabs
        type="border-card"
        v-model="tabsTwoActiveName"
        class="h100 form-tabs form-right-tabs"
      >
        <el-tab-pane label="字段属性" name="first" class="h100">
          <div :class="{ 'form-bg-two': !myArray.length }" class="h100">
            <el-scrollbar class="h100">
              <fieldAttribute
                :config="myArray[activeInf.index]"
                :components="components"
                v-if="myArray.length"
              ></fieldAttribute>
            </el-scrollbar>
        ...
        <el-tab-pane label="表单属性" name="second" class="h100">
          <div class="h100">
            <el-scrollbar class="h100">
              <formAttribute :config="formAttribute"></formAttribute>
            </el-scrollbar>
          </div>
        </el-tab-pane>
      </el-tabs>
  ...
  methods: {
    ...
    windowHeightChange() {
      ...
      if (evt.to.id == "widgetFormBox") {
        this.activeInf.prop =
          evt.newIndex != null ? this.myArray[evt.newIndex].prop : null;
        this.activeInf.index = evt.newIndex != null ? evt.newIndex : null;
      }
    },
    // 左侧复制到右侧
    cloneDog(original) {
